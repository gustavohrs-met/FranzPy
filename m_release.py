...
# Configuração padrão do Plotly (Copie exatamente assim)
PLOTLY_CONFIG = {
    'displayModeBar': True,
    'responsive': True
}
...
def plotar_modelos_ajustados(df_model_exp, df_resultados, t_data, q_data, y_label, T):
    ...
    # 1. Mudamos aqui para fig_fit
    fig_fit = go.Figure()
    
    # 2. Mudamos todos os 'fig.add_trace' para 'fig_fit.add_trace'
    ...
    try:
        if 'k0' in df_resultados.columns and pd.notna(df_resultados.loc['Zero-Order', 'k0']):
            ...
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_zero_order(t_fit, k0), mode='lines', name=T['model_zero_order'], line=dict(color=colors[0], dash='dash')))
            
        if 'k1' in df_resultados.columns and pd.notna(df_resultados.loc['First-Order', 'k1']):
            k1 = df_resultados.loc['First-Order', 'k1']
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_first_order(t_fit, q_max, k1), mode='lines', name=T['model_first_order'], line=dict(color=colors[1], dash='dash')))

        if 'kH' in df_resultados.columns and pd.notna(df_resultados.loc['Higuchi', 'kH']):
            kH = df_resultados.loc['Higuchi', 'kH']
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_higuchi(t_fit, kH), mode='lines', name=T['model_higuchi'], line=dict(color=colors[2], dash='dash')))
        ...
        if 'kKP' in df_resultados.columns and pd.notna(df_resultados.loc['Korsmeyer-Peppas', 'kKP']):
            ...
            fig_fit.add_trace(go.Scatter(x=t_kp, y=model_korsmeyer_peppas(t_kp, kKP, n), mode='lines', name=T['model_korsmeyer'], line=dict(color=colors[3], dash='dot')))
        ...
        if 'kHC' in df_resultados.columns and pd.notna(df_resultados.loc['Hixson-Crowell', 'kHC']):
            ...
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_hixson_crowell(t_fit, q_max, kHC), mode='lines', name=T['model_hixson'], line=dict(color=colors[4], dash='dash')))
        ...
        if 'a' in df_resultados.columns and pd.notna(df_resultados.loc['Weibull', 'a']):
            ...
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_weibull(t_fit, q_max, a, b), mode='lines', name=T['model_weibull'], line=dict(color=colors[5], dash='dash')))
        ...
        if 'k_diff' in df_resultados.columns and pd.notna(df_resultados.loc['Peppas-Sahlin', 'k_diff']):
            ...
            fig_fit.add_trace(go.Scatter(x=t_fit, y=model_peppas_sahlin(t_fit, k_diff, k_relax), mode='lines', name=T['model_peppas_sahlin'], line=dict(color=colors[6], dash='longdash')))
    
    except Exception as e:
        pass 
        
    # 3. Agora o bloco final está correto porque o gráfico se chama fig_fit
    fig_fit.update_layout(
        title=f"{T['step4_subheader_plot']}",
        xaxis_title=T['step3_xaxis_label'], 
        yaxis_title=y_label.split('(')[0].replace('Média_',''),
    ...
    st.plotly_chart(fig_fit, width='stretch', config=PLOTLY_CONFIG)
...
def render_step3(T):
    ...
    with col1:
        ...
        if grupo_selecionado:
            ...
            st.plotly_chart(fig_individual, width='stretch', config=PLOTLY_CONFIG)
    ...
    with col2:
        ...
        st.plotly_chart(fig_comparativo, width='stretch', config=PLOTLY_CONFIG)
